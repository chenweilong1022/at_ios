    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/ltt/cdlineip/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.ip = data.cdlineip.ip
                this.dataForm.deleteFlag = data.cdlineip.deleteFlag
                this.dataForm.createTime = data.cdlineip.createTime
                this.dataForm.tokenPhone = data.cdlineip.tokenPhone
                this.dataForm.lzCountry = data.cdlineip.lzCountry
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/ltt/cdlineip/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
