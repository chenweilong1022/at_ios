  ...
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('ltt:atusernametask:save')" type="primary" @click="addOrUpdateHandle()">新增</el-button>
        <el-button v-if="isAuth('ltt:atusernametask:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
      </el-form-item>
    </el-form>
    <el-table
      :data="dataList"
...
<script>
  import AddOrUpdate from './atusernametask-add-or-update'
  import ErrLogs from './atusernametask-err-logs.vue'
  export default {
    data () {
      return {
        dataForm: {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/ltt/atusernametask/list'),
          method: 'get',
      ...
      errRetryHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '错误重试' : '批量错误重试'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/ltt/atusernametask/errRetry'),
            method: 'post',
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/ltt/atusernametask/delete'),
            method: 'post',
            data: this.$http.adornData(ids, false)
          }).then(({data}) => {
            if (data && data.code === 0) {
